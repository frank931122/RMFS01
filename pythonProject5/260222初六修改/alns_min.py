...
def repair_greedy_insert_with_place(
    *,
    ...
    # ✅ 新增：cheap 预筛选（核心）
    preselect_m: int = 12,                 # 每个任务最多进入 evaluate 的候选 (r,pos,s) 数
    preselect_per_pos_shelves: int = 2,    # 每个 (r,pos) 只保留 cheap 最好的前 k 个回库位
    ...
    removed_list = [int(x) for x in removed]

    ws_fixed = getattr(evaluator, "ws_fixed_seq", {}) or {}
    ws_pos = _ws_index(ws_fixed)
    pi = getattr(evaluator, "pi", {}) or {}

    # ✅ shelf_seq ↔ v 一致性：提前准备 chain_of / shelf_idx（一次构建，内层复用）
    chain_of = _build_chain_of_map(shelf_seq, task_shelf_mapping)
    shelf_idx = _shelf_index(shelf_seq)
    ...
    # remaining_unfixed：当前 routes 中缺失的任务集合
    ...
    for j in removed_list:
        ...
        for r in agv_cands:
            ...
            for pos in pos_list:
                ...
                seq_ins = _normalize_one_route_ws_blocks(seq_ins, evaluator)

                # ✅ 核心：修同链任务相对顺序，使 v(由routes隐含) 与 shelf_seq 一致
                seq_ins = _normalize_one_route_shelf_slots(seq_ins, chain_of, shelf_idx)
                ...
                for s in cand_shelves:
                    ss = int(s)
                    ok, reason = pre.check_insert_candidate(
                        j=j,
                        r=r,
                        pos=pos,
                        end_s=ss,
                        ...
                        strict_move1=False, # 避免 normalize 带来的误剪
                    )
                    if ok:
                        ok_s.append(ss)
                    else:
        ...
        if not cheap_pool:
            ...
            routes[int(r_fb)] = _normalize_one_route_ws_blocks(routes[int(r_fb)], evaluator)
            # ✅ 兜底也要保证 v 与 shelf_seq 一致
            routes[int(r_fb)] = _normalize_one_route_shelf_slots(routes[int(r_fb)], chain_of, shelf_idx)
        ...
        preselect_m_eff = max(1, int(preselect_m))
        cheap_pool = cheap_pool[:preselect_m_eff]
        ...
        for _, rr, pp, ss, seq_ins in cheap_pool:
            ...
            try:
                ...
                obj_f = float(obj)
                if not math.isfinite(obj_f):
                    obj_f = 1e30  # inf 统一当超大，便于排序
                key = _infeas_key(det if isinstance(det, dict) else {}, evaluator)
                scored_cands.append((tuple(key), obj_f, int(rr), int(pp), int(ss), list(seq_ins)))
        ...
        if not scored_cands:
            ...
            # ✅ 关键：先按 infeas_key，再按 obj 排序
            scored_cands.sort(key=lambda x: (x[0], x[1]))
            kpick = min(max(1, int(top_k_random)), len(scored_cands))
            key_best, obj_best, rr, pp, ss, seq_ins = rng.choice(scored_cands[:kpick])

            routes[int(rr)] = list(seq_ins)
            place[j] = int(ss)

        #chosen = _choose_from_topk(best_cands, rng, top_k_random)
...
def repair_greedy_insert_with_place_ordered(
    *,
    ...
    """
    Ordered repair：按 removed_order 顺序逐个插回任务。

    关键升级：
      - max_evals_total：整个 ordered-repair 的总 evaluate 预算（跨任务共享）
      - max_evals_per_task：单任务预算上限（不会超过 total 还能分到的份额）
    """
    ...
    # 剩余总预算（None 表示不限）
    budget_left = None if max_evals_total is None else max(0, int(max_evals_total))

    removed_order = [int(x) for x in removed_order]

    for idx, j in enumerate(removed_order):
        j = int(j)

        # ---- 若总预算耗尽：走“零评估兜底插入”，保证解完整 ----
        if (budget_left is not None) and (budget_left <= 0):
            ...
            if not cand_shelves:
                cand_shelves = [min(int(s) for s in evaluator.S)]

            routes[int(r_fb)].append(j)
            routes[int(r_fb)] = _normalize_one_route_ws_blocks(routes[int(r_fb)], evaluator)
            place[j] = int(cand_shelves[0])
            continue

        # ---- 给当前任务分配一个“本任务预算” ----
        if budget_left is None:
            task_budget = None if max_evals_per_task is None else max(1, int(max_evals_per_task))
        else:
            tasks_left = len(removed_order) - idx
            share = max(1, budget_left // max(1, tasks_left))
            if max_evals_per_task is not None:
                share = min(share, max(1, int(max_evals_per_task)))
            task_budget = share

        # ---- 用单任务 repair（复用你已有逻辑）----
        routes, place = repair_greedy_insert_with_place(
            routes=routes,
            shelf_seq=shelf_seq,
            place=place,
            evaluator=evaluator,
            ...
            # ✅ 把本任务预算同时作为 per_task 和 total（单任务场景下等价）
            max_evals_per_task=task_budget,
            max_evals_total=task_budget,
            copy_inputs=False,
        )

        # 预算扣减：保守扣 task_budget（不会超用）
        if budget_left is not None and task_budget is not None:
            budget_left -= int(task_budget)
