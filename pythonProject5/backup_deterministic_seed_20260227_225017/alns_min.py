...
def alns_minimize(
    ...
    if turbo_mode:
        effective_iters = min(effective_iters, 220)
    ...
    if turbo_mode:
        effective_relabel_top_k = min(effective_relabel_top_k, 6)
    ...
    for it in range(effective_iters):
        ...
        p_min = 0.12 if turbo_mode else 0.15
        ...
        do_relabel = (
            (bool(relabel_when_stagnating) and stagnating)
            or (bool(relabel_when_strong_shake) and strong_shake)
            or periodic_relabel
        )
        if do_relabel:
            routes_rl, obj_rl = _relabel_best_routes(
