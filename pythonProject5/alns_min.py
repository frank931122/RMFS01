...
def alns_minimize(
    ...
    slim_runtime_mode_eff = bool(int(slim_runtime_mode or 0) == 1)
    ...
    if slim_runtime_mode_eff:
        eval_budget_heavy_eff = min(int(eval_budget_heavy_eff), max(300, int(0.45 * float(eval_budget_total_eff))))
    ...
    POST_SHAKE_ITERS = 18 if slim_runtime_mode_eff else 25
    post_shake = 0
    stall = 0
    ...
    for it in range(effective_iters):
        ...
        if fast_feasible_mode:
            if very_large_mode:
                ...
                improve_prob = 0.20
            if slim_runtime_mode_eff:
                light_stride = max(light_stride, 36 if very_large_mode else (24 if large_scale_mode else 14))
                improve_prob *= 0.55
            do_heavy_local = bool(
        ...
        if do_heavy_local:
            ...
            if slim_runtime_mode_eff:
                p_ejection = min(p_ejection, 0.18 if stagnating else 0.10)
            ...
            if slim_runtime_mode_eff:
                p_star = min(p_star, 0.18 if stagnating else 0.07)
            ...
            if slim_runtime_mode_eff:
                p_ws_micro = min(p_ws_micro, 0.16 if stagnating else 0.06)
            elif very_large_mode:
            ...
            if stagnating and ((it % (10 if slim_runtime_mode_eff else 6)) == 0):
